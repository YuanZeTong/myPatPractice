...
public class APlusBPolynomials {
 ...
 class Polynomial {
	...
	public Polynomial() {
		...
		String[] parameterStr = str.split(" ");
		
		k = Integer.parseInt(parameterStr[0]);
		nk = new double[k][2];
		
		for(int i = 1; i < parameterStr.length; i++) {
			if(i % 2 == 1)
				nk[(i - 1) / 2][0] = Double.parseDouble(parameterStr[i]);
			if(i % 2 == 0)
				nk[(i - 1) / 2][1] = Double.parseDouble(parameterStr[i]);
		}
	}
	...
	public void printSelfDate() {
		//打印多项式自己的参数
		StringBuilder outStr = new StringBuilder(k * 4 + 1);
		outStr.append("" + k);
		for(int i = 0; i < this.k; i++) {
			if(this.nk[i][1] == 0)
				continue;
			outStr.append(" " + (int)this.nk[i][0]);
			outStr.append(" " + (int)(this.nk[i][1] * 10) / 10.0);
		}
